...
void check_reorder_loss()
{
  ...
  for (count = 1; count < num_pkts_in_info; count++)
	...
	{
	  last_chirp = nc;
	  /*location of first packet*/
	  chirp_info[nc - first_chirp].first_pkt_loc = count;
	  /* Note: For chirps with reordered packets, "count" need not
	   * represent packet number 1. However we discard chirps with
	   * reordered packets and so wrongly marking the above is not
	   * a problem. */
	}
	...
	{
	  /* We mark packet_info because there is no entry in chirp_info
	   * for packets from "earlier" chirps that have arrived late. */
	  /* 2 is the value for reordering, will discard packet */
