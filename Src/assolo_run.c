...
void usage()
{
  fprintf(stderr,"usage: assolo_run -R <receiver> -S <sender> -t <duration(secs)> <more options, -h prints all options>\n \t -n <number of estimates to smooth over, default=11>\n \t -d <decrease factor (>1), default=1.5>\n \t -b <busy period length (integer >2), default=5>\n \t -U <receiver port (chirp UDP), default=%d (1024-65535)>\n \t -h Help: produces this output\n",SNDPORT);
  fprintf(stderr, "\t -R \t receiver host name or IP address \n");
  fprintf(stderr, "\t -S \t sender host name or IP address \n");
  fprintf(stderr, "\t -J \t number of packets per Jumbo packet,default=1. In case of packet coalescence use values larger than 1, e.g. -J 6 \n");
  fprintf(stderr, "\t -l \t lowest rate (Mbps) to probe at within chirp, default=%fMbps. NOTE: by default assolo will find an appropriate probing range.\n",DEFAULT_MIN_RATE);
  fprintf(stderr, "\t -u \t highest rate (Mbps) to probe at within chirp, default=%fMbps\n",DEFAULT_MAX_RATE);
  fprintf(stderr, "\t -e \t treshold , default=%fpercent\n",DEFAULT_SGL);
  fprintf(stderr, "\t -f \t filter , default=%d type\n",0);
  fprintf(stderr, "\t -p \t packet size <%d-%d>,default=1000 bytes\n",MINPKTSIZE,MAXMESG);
  fprintf(stderr, "\t -t \t duration of experiment(sec), default=600 sec \n");
  fprintf(stderr, "\t -s \t spread factor: ratio of consecutive inter-arrivals within a chirp, default=1.2 \n");
  fprintf(stderr, "\t -a \t average probing rate (Mbps), default=0.3Mbps \n");
...
void parse_cmd_line_run(int argc, char *argv[])
{
      ...
      while (*ptr)
	{
	  switch (*ptr++)
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no lowest rate '-l'.\n");
		      exit(1);
		    }
		  low_rate = atof(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no highest rate '-u'.\n");
		      exit(1);
		    }
		  high_rate = atof(*argv);
		}
	    ...
	    case 'a':		/* Average probing bandwidth */
	      if (*ptr == 0)
		{
		  argc--; argv++;
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no average rate '-a'.\n");
		      exit(1);
		    }
		  avg_rate = atof(*argv);
		}
	    ...
	    case 't':		/* Overall test duration */
	      if (*ptr == 0)
		{
		  argc--; argv++;
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no duration '-t'.\n");
		      exit(1);
		    }
		  duration = atof(*argv);
		}
	    ...
	    case 's':		/* Spread factor */
	      if (*ptr == 0)
		{
		  argc--; argv++;
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no spread_factor '-s'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no packet size given with '-p'.\n");
		      exit(1);
		    }
		  pktsize = atoi(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no Jumbo size given with '-J'.\n");
		      exit(1);
		    }
		  jumbo = atoi(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no destination host given with '-S'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no receiver host given with '-R'.\n");
		      exit(1);
		    }
		  strcpy (rxhostname, *argv);
		}
	      else
		{
		  strcpy (rxhostname, ptr);
		  *ptr = 0;
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no busy period length '-b'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no decrease factor '-d'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no averaging length '-n'.\n");
		      exit(1);
		    }
	      ...
	      if( *ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no port number given with '-U'.\n");
		      exit(1);
		    }
		  sndPort = atoi(*argv);
		}
	      ...
	      if( *ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no treshold '-e'\n");
		      exit(1);
		    }
		  soglia = atoi(*argv);
		}
	      ...
	      if( *ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_run: no filter '-f'\n");
		      exit(1);
		    }
		  filter = atoi(*argv);
		}
	    ...
	    default:
	      fprintf(stderr,"assolo_run: Unknown option '%c'\n", ptr[-1]);
	    }
	}
      argc--; argv++;
    }
  ...
  if (src_addr.s_addr == 0)
    {
      fprintf(stderr, "assolo_run: %s: unknown sender host\n",hostname);
      usage();
      exit(1);
    }
  else
  ...
  if (dest_addr.s_addr == 0)
    {
      fprintf(stderr, "assolo_run: %s: unknown receiver host\n",rxhostname);
      usage();
      exit(1);
    }
  else
  ...
  if (duration <0)
  ...
  else
    {
      ...
      stop_time = ( ((double) tp_start.tv_sec)
		    + (((double) tp_start.tv_usec) / 1000000.0) + duration );
    }
  ...
  if( low_rate < 0.0 || high_rate < 0.0 || avg_rate < 0.0
      || low_rate > high_rate)
    {
