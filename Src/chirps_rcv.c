...
inline void update_info(u_int32_t nc,
			u_int32_t np,
			double snd_time,
			double rcv_time,
			u_int32_t pkt_req_num,
			int good_jumbo_pkt)
{
...
void receive_chirp_pkts()
{
  ...
  u_int32_t np, nc;
  u_int32_t good_pkt_num = -1;		/* packet number received */
  u_int32_t good_chirp_num = -1;	/* chirp number received */
  u_int32_t prev_chirp = 0;		/* previous chirp number */
  u_int32_t prev_pkt_num = 0;
  u_int32_t prev_pkt_chirp = 0;
  ...
  while (1)
      ...
      if (ioctl(soudp, SIOCGSTAMP, (void *) tp) < 0)
	{
      ...
      if (cmptr != NULL && cmptr->cmsg_len == CMSG_LEN(sizeof(struct timeval)))
	{
      ...
      else
	{
      ...
      /* record packet info if it corresponds to latest request of rates,
       * if it is last non-coalesced packet of a particular rate*/
      if (ntohl(udprecord->request_num) == request_num - 1)
	{
	  if(jumbo > 1)
		...
		{
		  /* if the good packet has the previous packet number
		   * record it */
		  if (good_pkt_num == np-1)
		    {
		      update_info(good_chirp_num, good_pkt_num,
				  good_pkt_snd_time, good_pkt_rcv_time,
				  ntohl(udprecord->request_num), 1);
		    }
		  else
		    /* record last packet of previous jumbo */
		    if (prev_pkt_num == np-1)
		      {
			update_info(prev_pkt_chirp, prev_pkt_num,
				    prev_pkt_snd_time, prev_pkt_rcv_time,
				    ntohl(udprecord->request_num), 0);
			/* since there was no good packet only consider packets
			 * till this one*/
		      }
		    else
		      {
			/* no packets from np-1 jumbo so record good packet
			 * of last jumbo*/
			update_info(good_chirp_num, good_pkt_num,
				    good_pkt_snd_time, good_pkt_rcv_time,
				    ntohl(udprecord->request_num), 1);
	      ...
	      /* record first packet of last jumbo, not optimal
	       * but should not cause much trouble*/
	      if (np == num_interarrival+1
		  && prev_pkt_num < num_interarrival + 1)
		update_info(nc, np, cur_pkt_snd_time, cur_pkt_rcv_time,
			    ntohl(udprecord->request_num),1 );
      ...
      if (nc > prev_chirp)
	{
	  /* use latest wait time if chirp corresponds
	   * to latest parameters */
	  if (ntohl(udprecord->request_num) == request_num - 1)
	    {
	      wait_time.it_value.tv_sec = (long)floor(write_interval);
	      wait_time.it_value.tv_usec = (long)((double)((double)write_interval - (double)floor(write_interval)) * 1000000.0);
