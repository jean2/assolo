...
void create_arrays()
{
  /* The first part of the data packet can be accessed as the log record */
  udprecord			= (struct udprecord *) data;
  inst_bw_estimates_excursion	= (double *) calloc( (int) (num_inst_bw)
						     , sizeof(double) );
  qing_delay			= (double *) calloc( (int) (MAXCHIRPSIZE-1+1),
						     sizeof(double) );
  qing_delay_cumsum		= (double *) calloc( (int) (MAXCHIRPSIZE-1+1),
						     sizeof(double) );
  rates				= (double *) calloc( (int) (MAXCHIRPSIZE-1),
						     sizeof(double) );
  iat				= (double *) calloc( (int) (MAXCHIRPSIZE-1),
						     sizeof(double) );
  av_bw_per_pkt			= (double *) calloc( (int) (MAXCHIRPSIZE-1),
						     sizeof(double) );
  ...
  packet_info    = (struct pkt_info *)    calloc( pkts_per_write*2,
						  sizeof(struct pkt_info));
...
void update_rates_iat()
{
  ...
  k        = (num_interarrival - 1) / 2;	/* Loading vectors */
  rates[k] = ((high_rate + low_rate) / 2);
  thr      = (soglia / 100) * ((high_rate - low_rate) / 2);

  for (count = 0; count < k ; count++)
    {
      rates[k + count + 1] = (rates[k + count] + (thr * pow(spread_factor,
							    count)));
      rates[k - count - 1] = (rates[k - count] - (thr * pow(spread_factor,
							    count)));
    }
