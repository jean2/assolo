...
struct control_rcv2snd {
  ...
  /* The double values are recoded into an uint64 based on the
   * standard IEEE-754, after transmit they will be reassembled back to double.
   * This will mostly prevent the problem of different sizes and
   * representation of double on different platforms. */
  u_int64_t high_rate;		/* highest rate (Mbps) to probe at */
  u_int64_t inter_chirp_time;	/* time between chirps */
  u_int64_t low_rate;		/* lowest rate (Mbps) to probe at */
  u_int64_t soglia;		/* Threshold Percentage */
  u_int64_t spread_factor; 	/* decrease in spread of packets within the chirp */
