...
void parse_cmd_line_rcv(int argc, char *argv[])
{
  ...
  while (argc > 0)
      ...
      while (*ptr)
	{
	  switch (*ptr++)
	    ...
	    default:
	      fprintf(stderr, "assolo_rcv: Unknown option '%c'\n", ptr[-1]);
	    }
	}
      argc--; argv++;
    }
...
void parse_cmd_line(int argc, char *argv[])
{
      ...
      while (*ptr)
	{
	  switch (*ptr++)
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no lowest rate '-l'.\n");
		      exit(1);
		    }
		  low_rate = atof(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no highest rate '-u'.\n");
		      exit(1);
		    }
		  high_rate = atof(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no average rate '-a'.\n");
		      exit(1);
		    }
		  avg_rate = atof(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no duration '-t'.\n");
		      exit(1);
		    }
		  duration = atof(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no spread_factor '-s'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: receiver host name error '-R'.\n");
		      exit(1);
		    }
		}
	      break;
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no packet size given with '-p'.\n");
		      exit(1);
		    }
		  pktsize = atoi(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no treshold '-e'.\n");
		      exit(1);
		    }
		  soglia = atoi(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no filter '-f'.\n");
		      exit(1);
		    }
		  filter = atoi(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no Jumbo size given with '-J'.\n");
		      exit(1);
		    }
		  jumbo = atoi(*argv);
		}
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no destination host given with '-S'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no busy period length '-b'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr, "assolo_rcv: no decrease factor '-d'.\n");
		      exit(1);
		    }
	      ...
	      if (*ptr == 0)
		{
		  ...
		  if (*argv == 0)
		    {
		      fprintf(stderr,"assolo_rcv: no averaging length '-n'.\n");
		      exit(1);
		    }
  ...
  if (src_addr.s_addr == 0)
    {
      fprintf(stderr, "assolo_rcv: %s: unknown host\n",hostname);
      usage();
      exit(1);
    }
  else
  ...
  if (duration <0)
  ...
  else
    {
      ...
      stop_time = ( (double) tp_start.tv_sec
		    + (((double) tp_start.tv_usec) / 1000000.0)
		    + duration );
    }
...
{
  /* Used just in case given rates lead to an extra large chirp. */
  double new_high_rate;
  ...
  internal_computation = (high_rate + low_rate) / 2;
  while (internal_computation > low_rate)
    {
      internal_computation = internal_computation - (thr*pow(spread_factor, k));
      k++;
  ...
  rates_here = (double *) calloc((int)(MAXCHIRPSIZE-1), sizeof(double));
  iat_here = (double *) calloc((int)(MAXCHIRPSIZE-1), sizeof(double));

  rates_here[k] = ((high_rate + low_rate) / 2);

  for (count = 0; count < k; count++)
    {
      rates_here[k + count + 1] = ( rates_here[k + count]
  ...
  inter_chirp_time = ( ((double) (num_interarrival + 1)) * 8.0 * jumbo
  ...
  /* Allocate enough memory to store packet information if not enough */

  if ( (int) (1 + (write_interval / inter_chirp_time)) > chirps_per_write
       && created_arrays )
    {
      ...
      packet_info = realloc(packet_info,
			    pkts_per_write * 2 * sizeof(struct pkt_info));
      chirp_info = realloc(chirp_info,
			   chirps_per_write * 2 * sizeof(struct chirprecord));
    }
