...
void write_instant_bw(double inst_av_bw_excursion,double time_stamp, int nc)
{
  ...
  if (inst_bw_count>num_inst_bw)
      ...
      if(filter == 1)
	{
	  ...
	  for(g = 1; g < num_inst_bw; g++)
		...
		mx_inst = inst_bw_estimates_excursion[g];
	      den_vhf += fabs(inst_bw_estimates_excursion[g]
			      - inst_bw_estimates_excursion[g-1]);
	    }
      ...
      else
	{
      ...
      /* if estimates are close to either end of chirp probing rates,
       * change rates */
	...
	highcount=0;
	  ...
	  /* write to tcp socket */
	  if (write(new_so1, (char *) remote_pkt,sizeof(struct inst_bw_pkt))
	      < 0)
	    {
...
double compute_inst_bw_excursion()
{
  ...
  while (cur_loc <= max_good_pkt_this_chirp)
    {
      /* check if queuing delay has decreased to "almost"
       * what it was at cur_inc*/
      /* Marked as negative in case of coalescence, skip this packet*/
      if (qing_delay[cur_loc] > NEG_THRESH + 1.0)
	{
	  ...
	  if (qing_delay[cur_loc] - qing_delay[cur_inc]
	      < (max_q / decrease_factor))
	    {
	      if (cur_loc-cur_inc >= busy_period_thresh)
		{
		  for (count = cur_inc; count <= cur_loc - 1; count++)
		    {
		      /* Mark all increase regions between cur_inc and cur_loc
		       * as having available bandwidth equal to their rates */
    ...
    /* Mark the available bandwidth during the last excursion
     * as the rate at its beginning */
  ...
  for (count = 0; count < num_pkts_in_info; count++)
    {
      ...
      /* If packet reorder */
      if (chirp_info[nc - first_chirp].reorder == 2
	  || packet_info[count].context_switch == 2)
	continue;      
      else /* no reorder */
	{
	  /* if loss */
	  if (chirp_info[nc - first_chirp].num < num_interarrival + 1)
		  ...
		  if (last_chirp > first_chirp)
		    {
		      /* if not first or last chirp */
		      if (nc > first_chirp
			  && nc < last_chirp
			  && np == chirp_info[nc - first_chirp].last_pkt)
			num_bad_chirps++;
		    }
		  else
		    {
		      if (np == chirp_info[nc - first_chirp].last_pkt)
			write_instant_bw(low_rate * 1000000.0 / 2.0,
	      ...
	      if (np > 1)
		{
		  ...
		  if (packet_info[count].good_jumbo_pkt == 0
		  ...
		  if (packet_info[count].rcv_time - packet_info[prev_pkt].rcv_time < context_receive_thresh)
		    {
		      ...
		      if (num_consec_bad >= CONSEC_BAD_PKTS
			  && max_good_pkt_this_chirp == num_interarrival)
			{
	      ...
	      else
		{
	      ...
	      /* Location of previous packet in this "good" chirp */
	      prev_pkt = count;
	      ...
	      if (np == (num_interarrival + 1))
		{
		  if (context_switch == 1)
		  ...
		  else
		    {
		      ...
		      write_instant_bw(inst_bw_excursion,
				       packet_info[count].rcv_time, nc);
		    }
...
int check_for_new_pars()
{
   ...
   * This Code is bugged. It will force the high rate to update
   * and then produce constant estimates since the factor
   * of the adjustment seems to be to great. (Tested on 10Mbit Link)
   ...
   */
  /* If you use assolo 0.9a then you will encounter a problem.
   * If the Upper and LowerBorder is adjusted automatically
   * the Upper Border will be divided by the factor 10.000.
   * This will cause LowerBorder > UpperBorder => very bad ;)
   * Someone forgot about the 10.000 Factor which is used
   * in computation of the Reach Stream (Receiver Code)
   *
  ...
  if (0) /* was if (lowcount > 2) */
    {
      /* Less than 1Mbps, have smaller range of rates, that is fewer packets
       * per chirp. This allows us to send more chirps per unit time. */
      if (low_rate <= 1.0) 
	{
  ...
  if(0) //was if (highcount > 2 && high_rate < 0.9 * (double) MAX_HIGH_RATE)
    {
      if (high_rate > 1.0)
	{
